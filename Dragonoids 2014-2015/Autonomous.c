...
task main() {
  ...
  eraseDisplay();
	...
	const int raiseToCentimeters = 56;
	const int lowerToCentimeters = 10;
	int sonarValue = 0;
	const int maxFailures = 10000;
	int failures = 0;
	while (true) {
		sonarValue = SensorValue[armHeight];
		if (sonarValue >= 200) {
			// Read error
			sonarValue = 0; // Don't break the loop yet
			failures++;
		...
		if (SensorValue[armHeight] >= raiseToCentimeters) {
			break;
		}
	...
	// Wait for them to reach that position
	while(ServoValue[leftGrabber] > servoMeasuringThreshold && ServoValue[rightGrabber] < (255 - servoMeasuringThreshold)) {}
	// Clear the tube before lowering the arm
	goForwardPower(750, 20);
	servo[tilt] = 110;
	while(SensorValue[armHeight] > lowerToCentimeters) {
